...
from app.models.todo import (  # Importing the Todo model from another file
    Todo,
    TodoUpdate,
)
...
class Database:
    ...
    async def get(self, id: PydanticObjectId) -> Any:
        ...
        doc = await self.model.get(
            id
        )  # Query the database using the model's get method
    ...
    async def update(self, id: PydanticObjectId, body: TodoUpdate) -> Any:
        ...
        des_body = body.dict()
        des_body = {
            key: value for key, value in des_body.items() if value is not None
        }  # Remove None values
